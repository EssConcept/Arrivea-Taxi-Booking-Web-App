...
<body>
    ...
    if(isset($_GET['driver_id'])){
        ...
        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo "<table>
                <tr>
                    <td><img src='$pfp' height='100px' width='100px'></td>
                    <td>$surname $name</td>
                    <td>$email</td>
                    <td>$phone_number</td>
                    <td>$email</td>
    ...
    <div>
        <form method="POST">
            <label for="confirm">Check the box and submit to kick your driver</label>
            <input type="checkbox" value="agree" name="confirm" id="confirm" required>
            <br>
            <input type="submit">
        </form>
    </div>
