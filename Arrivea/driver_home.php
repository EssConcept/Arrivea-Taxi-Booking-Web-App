...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['passanger_id']) && (isset($_POST['accept_drive']) || isset($_POST['reject_drive']))) {
    $passanger_id = $_POST['passanger_id'];
    $confirmation_status = isset($_POST['accept_drive']) ? 'confirmed' : 'rejected';
...
/*
// za brisanje iz tabele, ce je rejected
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['passanger_id']) && isset($_POST['reject_drive'])) {
    $passanger_id = $_POST['passanger_id'];

    // Poizvedba za brisanje vožnje iz tabele 'drive'
    $delete_query = "DELETE FROM drive WHERE passanger_id = $passanger_id";
    $delete_result = mysqli_query($con, $delete_query);

    if ($delete_result) {
        echo "Drive request rejected successfully.";
    } else {
        echo "Error rejecting drive request.";
    }
}*/
?>
<style>
